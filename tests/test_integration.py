...
class TestVariables(unittest.TestCase):
    ...
    def test_assign_conditional_elif(self):
        returned_val, returned_err = run(
            "<stdin>", "VAR A = IF 2==5 THEN 2 ELIF 3==5 THEN 3 ELSE 5"
        )
        self.assertEqual("5", str(returned_val))
...
class TestLists(unittest.TestCase):
    ...
    def test_list_in_conditional(self):
        returned_val, returned_err = run(
            "<stdin>", "IF 1 == 1 THEN [1, 2, 3] ELSE [4, 5, 6]"
        )
        self.assertEqual("[1, 2, 3]", str(returned_val))
        self.assertIsNone(returned_err)
...
class TestMaps(unittest.TestCase):
    ...
    def test_map_with_strings(self):
        returned_val, returned_err = run("<stdin>", '{"name": "Alice", "age": "25"}')
        self.assertEqual('{"name": "Alice", "age": "25"}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_with_numbers(self):
        returned_val, returned_err = run("<stdin>", '{"x": 10, "y": 20}')
        self.assertEqual('{"x": 10, "y": 20}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_with_mixed_types(self):
        returned_val, returned_err = run(
            "<stdin>", '{"name": "Bob", "age": 30, "score": 95.5}'
        )
        self.assertIsNotNone(returned_val)
        self.assertIsNone(returned_err)

    def test_map_single_entry(self):
        returned_val, returned_err = run("<stdin>", '{"key": "value"}')
        self.assertEqual('{"key": "value"}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_in_variable(self):
        run("<stdin>", 'VAR person = {"name": "Alice", "age": 25}')
        returned_val, returned_err = run("<stdin>", "SHOW person")
        self.assertEqual('{"name": "Alice", "age": 25}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_get_value(self):
        returned_val, returned_err = run(
            "<stdin>", '{"name": "Alice", "age": 25} / "name"'
        )
        self.assertEqual('"Alice"', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_get_number_value(self):
        returned_val, returned_err = run("<stdin>", '{"x": 100, "y": 200} / "x"')
        self.assertEqual("100", str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_add_entry(self):
        returned_val, returned_err = run("<stdin>", '{"a": 1} + {"b": 2}')
        self.assertEqual('{"a": 1, "b": 2}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_update_entry(self):
        returned_val, returned_err = run("<stdin>", '{"a": 1, "b": 2} + {"b": 99}')
        # Should update b to 99
        self.assertIn("99", str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_remove_entry(self):
        returned_val, returned_err = run("<stdin>", '{"a": 1, "b": 2, "c": 3} - "b"')
        result_str = str(returned_val)
        self.assertIn('"a"', result_str)
        self.assertIn('"c"', result_str)
        self.assertNotIn('"b"', result_str)
        self.assertIsNone(returned_err)

    def test_map_merge(self):
        run("<stdin>", 'VAR map1 = {"a": 1, "b": 2}')
        run("<stdin>", 'VAR map2 = {"c": 3, "d": 4}')
        returned_val, returned_err = run("<stdin>", "map1 * map2")
        result_str = str(returned_val)
        self.assertIn('"a"', result_str)
    ...
    def test_map_in_conditional(self):
        returned_val, returned_err = run(
            "<stdin>", 'IF 1 == 1 THEN {"yes": 1} ELSE {"no": 0}'
        )
        self.assertEqual('{"yes": 1}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_with_function(self):
        run("<stdin>", 'FUNC get_map() -> {"x": 10, "y": 20}')
        returned_val, returned_err = run("<stdin>", "get_map()")
        self.assertEqual('{"x": 10, "y": 20}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_operations_in_variable(self):
        run("<stdin>", 'VAR map1 = {"a": 1}')
        returned_val, returned_err = run("<stdin>", 'VAR map2 = map1 + {"b": 2}')
        self.assertEqual('{"a": 1, "b": 2}', str(returned_val))
        self.assertIsNone(returned_err)

    def test_map_with_list_values(self):
        returned_val, returned_err = run("<stdin>", '{"nums": [1, 2, 3]}')
        self.assertIsNotNone(returned_val)
        self.assertIsNone(returned_err)

    def test_map_nested(self):
        returned_val, returned_err = run("<stdin>", '{"outer": {"inner": 42}}')
        self.assertIsNotNone(returned_val)
        self.assertIsNone(returned_err)


class TestMapErrors(unittest.TestCase):
    """Tests for map error handling."""

    def test_map_key_not_found(self):
        returned_val, returned_err = run("<stdin>", '{"a": 1} / "b"')
        self.assertIsNone(returned_val)
        self.assertIsInstance(returned_err, RTError)
        self.assertIn("does not exist", returned_err.details)

    def test_map_remove_nonexistent_key(self):
        returned_val, returned_err = run("<stdin>", '{"a": 1} - "b"')
        self.assertIsNone(returned_val)
        self.assertIsInstance(returned_err, RTError)
        self.assertIn("does not exist", returned_err.details)

    def test_empty_map_key_access(self):
        returned_val, returned_err = run("<stdin>", '{} / "key"')
        self.assertIsNone(returned_val)
        self.assertIsInstance(returned_err, RTError)
